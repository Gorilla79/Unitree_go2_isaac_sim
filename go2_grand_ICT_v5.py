...
class GrandICT(Node):
    def __init__(self):
        ...
        self.pub_danger = self.create_publisher(String, 'danger', 10)
        self._danger_timer = self.create_timer(0.2, self._tick_danger)
    ...
    def _tick_danger(self):
        msg = String()
        msg.data = "Red" if (self.target_id is not None) else "White"
        self.pub_danger.publish(msg)
    ...
    def loop(self):
        ...
        danger_msg = String()
        danger_msg.data = "red" if (self.target_id is not None) else "white"
        ...
        key = cv2.waitKey(1) & 0xFF   # ← key 정의

        # --- r 키 입력 시 상태 초기화 ---
        if key == ord('r'):
            self.get_logger().info("[KEY] Reset triggered (r)")
            self.target_id = None
            self.danger_count.clear()
            self.history.clear()
            self._danger_announced = False
